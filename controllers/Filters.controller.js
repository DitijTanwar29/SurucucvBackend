...
const getFilterOptions = async (userType) => {
    ...
    const servicesFilter = {
        ...
        defaultOptions: Object.fromEntries(
            servicesList.map(service => [service._id, { label: service.serviceName }])
        )
    };
...
const buildQueryFromFilters = (filters) => {
    ...
    const filterMapping = {
        ...
        srcType: (value) => {
            const srcConditions = value.map(src => {
        ...
        additionalTraining: (value) => {
            const trainingConditions = value.map(training => {
                if (training === "blindSpot") return { isBlindSpotTraining: true };
                if (training === "safeDriving") return { isSafeDrivingTraining: true };
                if (training === "fuelEconomy") return { isFuelEconomyTraining: true };
                return null;
            }).filter(Boolean);
        ...
        history: (value) => {
            ...
            return historyMap[value] ? { publishedDate: { $gte: historyMap[value] } } : null;
        }
